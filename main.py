...
def checkAndResetValue(path, value_name, original_value, datatype):
    ...
    if key:
        ...
        if str(value) != str(original_value) and input(f"The registery value {value_name} at {path} is set to {str(value)} instead of {original_value}. Do you want to reset it? (y/n) ") == 'y':
